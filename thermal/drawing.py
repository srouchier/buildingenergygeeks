...
# Un cran au dessus
h = 1.5
pgm.add_node(daft.Node("T",   r"$T$",   2, h, plot_params = {"ec": "#ff0000"}))
pgm.add_node(daft.Node("epsT", r"$\sigma^T$", 3, h))
# Un cran au dessus
h = 2.5
...
# Tout en haut
h = 3.5
pgm.add_node(daft.Node("obsU", r"$u^\mathrm{obs}$", 0.5 , h, observed = True, aspect = 1.5))
pgm.add_node(daft.Node("epsU", r"$\sigma^u$", 1.5 , h))
...
pgm.figure.savefig("dessin_thermo.pdf")
pgm.figure.savefig("dessin_thermo.png", dpi=300)
