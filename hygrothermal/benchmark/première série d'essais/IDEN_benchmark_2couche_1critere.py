...
from inversion.benchmark_input_2couches_24h import mesh, clim, init, temps
...
parametres.append( {"lambda_0"   : [0.8, 2,5],
                    "lambda_mst" : [1, 3],
                    "lambda_tmp" : [5e-5, 1.5e-4],
                    "cp"         : [500, 1500],
                    "dp_025"     : [3e-12, 8e-12],
                    "dp_075"     : [5e-12, 1.5e-11],
                    "w_025"      : [25, 35],
                    "w_050"      : [35, 45],
                    "w_075"      : [45, 55] } )
# Intervalles de recherche initiaux du deuxième matériau
...
capteurs = [0, 0.10, 0.18]

# On spécifie le fichier des mesures et le nom des colonnes à y lire
mesures = pd.read_csv('D:\MCF\Simulation\Python\inversion/benchmark_mesure_2couches_HR2.txt', delimiter='\t')
t_tags  = 't (s)'
T_tags  = ['T (x=0cm)', 'T (x=10cm)', 'T (x=18cm)']
HR_tags = ['HR (x=0cm)', 'HR (x=10cm)', 'HR (x=18cm)']
...
for m in NMATER:
    prop_init.append( {'lambda_0'   : materiaux_initiaux[m].lambda_0,
                       'lambda_mst' : materiaux_initiaux[m].lambda_mst,
                       'lambda_tmp' : materiaux_initiaux[m].lambda_tmp,
                       'cp'         : materiaux_initiaux[m].cp,
...
def evaluation(individual):
    ...
    if not type(resultat_simul)==dict:
        ...
        T_calcul  = np.column_stack( HAM.postpro.evolution(resultat_simul, 'T',  x = p, t = t_mesure) for p in capteurs )
        ...
        R_T  = np.sum( (T_mesure -  T_calcul) **2 ) / np.mean(T_mesure)**2
...
def main():
    ...
    # Nombre max de générations si la convergence n'est pas atteinte avant
    NGENMAX = 1000
    
    # Enregistrement de la stratégie
    strategy = cma.Strategy(centroide_initial, sigma=0.5)
    ...
    # Algorithme
    for gen in xrange(NGENMAX):
        # Generate a new population
        population = toolbox.generate()
        # Ecart type de chaque parametre de la population (normé sur la moyenne)
        stats_pop[gen] = np.abs( np.std(np.array(population),axis=0) / np.mean(np.array(population),axis=0) )
...
if __name__ == "__main__":
    ...
    np.savetxt('CMA_ES_meilleurindividu.txt', meilleur)
    np.savetxt('CMA_ES_meilleurindividu_tag.txt', OptGlobal[0].keys(), fmt="%s")
    total = np.column_stack((sigma_f, moyenne, minimum, OptParGen, Ecartypes))
    np.savetxt('CMA_ES_touslesresultats.txt', total)