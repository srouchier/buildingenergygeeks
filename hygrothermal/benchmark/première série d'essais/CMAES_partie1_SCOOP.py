...
if len(capteurs) != len(T_tags):
    Exception('Nombres de capteurs incompatibles')

# Capteurs réels de température
precision_T = 0
erreur_T = precision_T/2*np.random.randn(len(T_mesure),len(capteurs))
...
prop_init.append( {'lambda_0'   : materiaux_initiaux[0].lambda_0,
                   'lambda_mst' : materiaux_initiaux[0].lambda_mst,
                   'lambda_tmp' : materiaux_initiaux[0].lambda_tmp,
                   'cp'         : materiaux_initiaux[0].cp,
                   'w_025'      : 10,
                   'w_050'      : 15,
                   'w_075'      : 25,
                   'dp_025'     : 12e-11,
...
def main():
    ...
    strategy = cma.Strategy(centroide_initial, sigma=0.5, lambda_=8)
...
if __name__ == "__main__":
    ...
    np.savetxt('CMAES_partie1_bestfit.txt', meilleur)
    np.savetxt('CMAES_partie1_bestfit_tag.txt', OptGlobal[0].keys(), fmt="%s")
    total = np.column_stack((sigma_f, moyenne, minimum, OptParGen[:gen], Ecartypes[:gen]))
    np.savetxt('CMAES_partie1_stats.txt', total)
